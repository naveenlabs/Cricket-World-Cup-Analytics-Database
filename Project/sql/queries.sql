-- =====================================================
-- Q1:
-- Among the top run-scorers in World Cup history,
-- which players combine frequent high-impact innings
...
-- Total World Cup runs per player (eligibility filter)
...
-- Select elite batters per format
top_run_scorers AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
...
-- Compute impact metrics for elite batters only
player_impact AS (
    SELECT
        pmr.format_name,
        pmr.player_id,
...
-- Rank players by sustained impact
ranked_players AS (
    SELECT
        format_name,
        player_name,
...
-- =====================================================
-- Q2:
-- Which batters score the most runs in each match phase
-- (powerplay, middle overs, death overs) of ODI and T20
-- World Cups, and how efficient are they in terms of
-- strike rate?
-- =====================================================
...
-- Identify elite batters (top 15 run-scorers per format)
elite_batters AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
...
-- Assign match phase to each delivery
phase_deliveries AS (
    SELECT
        f.format_name,
        CASE
...
-- Apply phase-aware volume thresholds and rank by runs
...
-- Final result: top 5 run-scorers per format and phase
SELECT
    format_name,
    match_phase,
    player_name,
...
-- =====================================================
-- Q3:
-- How does strike rate evolve across innings phases
-- for top-order vs middle-order batters in ODI and T20
-- World Cups?
-- =====================================================

WITH batter_entry AS (
    -- Determine batting order based on first ball faced
    SELECT
        i.innings_id,
...
-- =====================================================
-- Q4:
-- Powerplay contribution to team scoring in Wins vs
-- Losses across ODI and T20 World Cups
-- =====================================================
...
-- =====================================================
-- Q5:
-- How does wicket-taking effectiveness vary between
-- powerplay and death overs across ODI and T20 World Cups,
-- and which bowlers specialise in each phase?
-- =====================================================
...
WITH phase_deliveries AS (
    ...
    -- bowling team = match team that is NOT batting team
    JOIN MatchTeam mt
...
-- =====================================================
-- Q6:
-- What dismissal types dominate in ODI versus T20 World Cups,
-- and how does their distribution change across match phases?
-- =====================================================
...
-- =====================================================
-- Q7:
-- Which players have the highest impact performances
-- in World Cup knockout matches when combining
...
-- =====================================================
-- Q8 (Revised):
-- How do fielding contributions (catches and run-outs)
-- vary between ODI and T20 World Cups, and which teams
-- benefit most from fielding events?
-- =====================================================
...
-- all team appearances in knockout matches
...
-- fielding events (may be zero per match)
fielding_events AS (
    SELECT
        km.format_name,
        mt.team_id,
...
-- =====================================================
-- Q9:
-- How does team scoring progression by phase differ
-- between ODI and T20 World Cups, and what structural
-- differences emerge between formats?
--
-- Metric: Run rate (runs per over)
-- Teams: Top 5 teams by total runs per format
-- =====================================================
...
-- =====================================================
-- Q10:
-- How does average team scoring progression differ
-- between 1st and 2nd innings in ODI and T20 World Cups
-- across venues?
-- =====================================================
...
WITH innings_scores AS (
    SELECT
        ...
        -- Canonical venue name to merge duplicates
...
venue_innings_avg AS (
    SELECT
        ...
        -- Combine inconsistent country labels transparently
        GROUP_CONCAT(
...
-- =====================================================
-- Q11:
-- How does bowling economy in death overs compare between
-- ODI and T20 World Cups, and which bowlers consistently
-- limit scoring under end-of-innings pressure?
-- =====================================================
...
-- =====================================================
-- Q12:
-- How does team winning probability vary with top-order
-- stability in ODI and T20 World Cups?
-- =====================================================
...
-- =====================================================
-- Q13:
-- What proportion of total runs in ODI and T20 World Cups
-- comes from extras, and how does the type of extras
-- conceded differ between formats and teams?
-- =====================================================
...
-- =====================================================
-- Q14:
-- How efficiently do teams convert run chases into wins
-- in ODI and T20 World Cups, based on chase success rate
-- and run-rate efficiency.
-- =====================================================
...
WITH chase_level AS (
    SELECT
        ...
        /* Total runs scored in successful chases */
        SUM(d.runs_batsman + IFNULL(de.extra_runs, 0)) AS runs_scored,

        /* Legal balls faced (exclude wides and no-balls) */
        COUNT(
            CASE
...
team_aggregate AS (
    SELECT
        ...
        /* Achieved run rate during successful chases */
        AVG((runs_scored / legal_balls) * 6) AS avg_achieved_rr,

        /* Approximate required run rate based on format */
        AVG(
            CASE
...
-- =====================================================
-- Q15:
-- How does wicket loss timing affect scoring momentum
-- in ODI and T20 World Cups, measured by run rate
-- before and after wickets at the team level?
-- =====================================================
